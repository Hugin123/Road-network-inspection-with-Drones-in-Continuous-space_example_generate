...
# 无人机参数（定死）
# ============================================================
DRONE_BATTERY     = 1000      # 无人机电池容量 kwh
...
DRONE_DRIVE_COST  = 10000     # 无人机行驶成本系数 元/kwh
BIG_M = 10000                 # 足够大的常数 M

# ============================================================
# 坐标缩放参数：将归一化坐标映射到几十~几百米
# ============================================================
COORD_SCALE = 200             # 大规模路网坐标范围适当放大

# ============================================================
# 基站位置偏移系数（相对路网包围盒）
# ============================================================
DEPOT_OFFSET = 0.3            # 基站距路网边界的偏移比例
...
def compute_depot_pos(pos, direction):
    ...
    x_span = max((x_max - x_min) * DEPOT_OFFSET, 0.5)
    y_span = max((y_max - y_min) * DEPOT_OFFSET, 0.5)
...
def save_network_figure(fig_path, G, pos, all_depot_positions):
    ...
    nx.draw_networkx_nodes(G, pos_tuple, ax=ax, node_color='#27AE60',
                           node_size=250, linewidths=0.8, edgecolors='white')
...
if __name__ == "__main__":
    ...
    total_figs  = 0
    for mode in MODES:
        for (road_nodes, edges) in LARGE_CONFIGS:
